...
| E4, 2026-08-18 | iOS sim                    | C-E pass. The offline day-in-the-life found three: a failed sites load hid the on-site card so a cleaner could not check out; Start checks and Check out shared a busy flag, so tapping the former underground froze the latter for the 20s request timeout; and a permanently-failing attendance push would have retried forever. All fixed here |
| 1.0.2 pre-submission, 2026-08-31 | Android emulator (radios cut for real offline) + iOS sim | A-D and F largely pass on v1.0.2 build 10 code, including the full offline capture-submit-forcestop-selfsend loop with a photo, run against the ZZ REVIEW sandbox. E not run (no cleaner sandbox account); camera paths not simulable; D13 (sign out, straight back in) blocked on iOS by a simulator QUIC failure, not attributable to the app (Android passed it same minute). Found FIND-013 (photo rows survive a Fail-to-Pass change). Outstanding for a real-device pass before submitting: D13 and the camera items |
